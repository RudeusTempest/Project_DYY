...
class ExtractionService:
    ...
    @staticmethod
    def extract_bandwidth_per_interface_juniper(interface_output):
        """
        ...
        - bandwidth_max_mbps: Maximum configured bandwidth (in Mbps)
        - mtu: Maximum Transmission Unit size
        - input_errors: Input errors count
        - output_errors: Output errors count
        """
        try:
            # Extract Speed (max bandwidth)
            bandwidth_match = re.search(r"Speed:\s+(\d+)([MG]bps)", interface_output)
            if bandwidth_match:
                bandwidth_value = int(bandwidth_match.group(1))
                bandwidth_unit = bandwidth_match.group(2)
                bandwidth_max_mbps = bandwidth_value * 1000 if bandwidth_unit == "Gbps" else bandwidth_value
            else:
                bandwidth_max_mbps = None
            ...
            # Extract errors
            errors_match = re.search(r"Errors:\s+(\d+)", interface_output)
    ...
    @staticmethod 
    def extract_cisco(device_type, hostname_output, ip_output, mac_output, info_neighbors_output, all_interfaces_output=None):
        """
